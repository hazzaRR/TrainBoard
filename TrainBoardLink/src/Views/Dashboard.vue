  ...
  <div class="container mb-5">
    <div class="card m-4">
      ...
      <div class="card-body w-100 mx-auto">
        <div class="m-2 mapContainer">
          <StationMap :stations="stations"
            v-model:selected-point="crs"/>
        </div>
        <div class="form-floating mb-3">
          <input type="number" class="form-control" id="floatingNumRows" min="1" max="10" disabled v-model="numRows" />
          <label for="floatingNumRows">Number of Departure rows</label>
        </div>
        <div :class="['form-floating', 'mb-3']">
          <div :class="['form-control', 'multiSelect']">
            <Multiselect id="crsStationMultiSelect" v-model="crs" :searchable="true"
              :custom-label="(station) => `${station.name} (${station.crs})`" :options="stations"
              :close-on-select="true" :allow-empty="false" track-by="crs" />
          </div>
          <label for="crsStationMultiSelect" class="form-label">Station CRS</label>
        </div>
        <div :class="['form-floating', 'mb-3', , 'input-group']">
          <div :class="['form-control', 'multiSelect']">
            <Multiselect id="filterCrsStationMultiSelect" v-model="filterCrs" :searchable="true"
              :custom-label="(station) => `${station.name} (${station.crs})`" :options="stations"
              :close-on-select="true" track-by="crs" />
            <button class="btn btn-primary" @click="() => (filterCrs = null)">
              Clear
            </button>
          </div>
          <label for="filterCrsStationMultiSelect" class="form-label">Destination station CRS</label>
        </div>
        <div class="form-floating mb-3">
          <select class="form-control" id="floatingFilterCrs" min="1" v-model="filterType">
        ...
        <div class="form-floating mb-3">
          <input type="number" class="form-control" id="floatingTimeOffset" min="1" max="120" v-model="timeOffset" />
          <label for="floatingTimeOffset">Time Offset</label>
        </div>
        <div class="form-floating mb-3">
          <input type="number" class="form-control" id="floatingTimeWindow" min="1" max="120" v-model="timeWindow" />
        ...
        <div class="form-floating mb-3">
          <input type="color" class="form-control" id="floatingPlatformColour" v-model="platformColour" />
          <label for="floatingPlatformColour">Platform Colour</label>
        </div>
        <div class="form-floating mb-3">
          <input type="color" class="form-control" id="floatingDestinationColour" v-model="destinationColour" />
          <label for="floatingDestinationColour">Destination Colour</label>
        </div>
        <div class="form-floating mb-3">
          <input type="color" class="form-control" id="floatingCallingPointsColour" v-model="callingPointsColour" />
          <label for="floatingDestinationColour">Calling Points Colour</label>
        </div>
        <div class="form-floating mb-3">
          <input type="color" class="form-control" id="floatingCurrentTimeColour" v-model="currentTimeColour" />
          <label for="floatingDestinationColour">Current Time Colour</label>
        </div>
        <div class="form-floating mb-3">
          <input type="color" class="form-control" id="floatingOnTimeColour" v-model="onTimeColour" />
          <label for="floatingOnTimeColour">On Time Colour</label>
        </div>
        <div class="form-floating mb-3">
          <input type="color" class="form-control" id="floatingDelayColour" v-model="delayColour" />
...
function resetMatrixConfig() {
  numRows.value = 1;
  crs.value = { name: "Colchester", crs: "COL", owner: "Greater Anglia", latitude: "51.900711", longitude: "0.892598" };
